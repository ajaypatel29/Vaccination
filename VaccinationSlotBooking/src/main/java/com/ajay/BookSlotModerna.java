...
/**
 * Servlet implementation class BookSlotModerna
 */
@WebServlet("/BookSlotModerna")
public class BookSlotModerna extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public BookSlotModerna() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			st1.setString(2, "Moderna");
			st.executeUpdate();
			st1.executeUpdate();
			cst2.execute(("update vaccines set Moderna = Moderna - 1  where date = curdate()"));
